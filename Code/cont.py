...
class Contact:
    ...
    def phoneDict(self):
        if Contact.pDictRefresh==True:
            for name in self.contacts:
                for phone in self.contacts[name]['number']:
                    Contact.pDict[phone]=name
            Contact.pDictRefresh==False        
...
class Console:
    ...
    def showHelp(self,showWhich=[]):
        ...
        arr = [
            [
                "List Contacts: L [-fe [tag1, tag2, ...]] [-f [tag1, tag2, ...]] [custom field]",
                "  -f: Filter tags",
                "  -fe: Exclude tags",
                "  [custom field]: categorize by one custom field"
            ],
            [
            ...
            [
                ...
                "  Quotion mark is needed"
            ],
            [
                "Modify Contact: M -n [name] [-p [phone newphone] [-o [phone1, phone2, ...]]] [-e [email newemail] [-o [email1, email2, ...]] [-c tag [category] [-o [category1, category2, ...]]]",
    ...
    def exec(self,command:str):
        ...
        while(ind+1<strLen):
            ind+=1
            if(not shifting and (command[ind]=="\"" or command[ind]=="'")):
                # type of quote marks:
                #    "''"
                #    "'"
            ...
            if(qLen==0):
                ...
                if(shifting):
                    ...
                    for j in range(1,6,2):
                        _esca=JSON.escapeList.get(command[ind:ind+j])
                        if(_esca):
                            tmpValue+=_esca
                            ind+=j-1
                            break
        ...
        if(mode=="L"):
            ...
            if(not self.con.contacts.get(name) and update):
                ...
                return True                     #non-exist
            if(not silence and self.con.contacts.get(name) and not cover and not manualMode):
                    ...
                    if(not silence):
                        while(True):
                            ...
                            if(i==0 and (not re.match("^[0-9]*$",pNumber) or len(pNumber)==0)):
                                ...
                                break
                            if pNumber in self.con.pDict:
                                    print("Already exists, Delete previous one? (Y/N)")
                                    if(self.con.getUserInput()):
                                        self.con.contacts[self.con.pDict[pNumber]]['number'].remove(pNumber)
                                        self.con.pDict[pNumber]=name
            ...
            return (not self.con.getUserInput())      
        else:
